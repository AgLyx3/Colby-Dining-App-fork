...
class DiningHallPredictor:
    """
    Class name: DiningHallPredictor
    """
    ...
    def save_models(self):
        ...
        for h in self.dining_halls:
            if h in self.models and h in self.scalers:
                try:
                    model_path = os.path.join(self.model_dir, f'{h.lower()}_model.joblib')
                    scaler_path = os.path.join(self.model_dir, f'{h.lower()}_scaler.joblib')
                    joblib.dump(self.models[h], model_path)
                    joblib.dump(self.scalers[h], scaler_path)
                    logger.info("Saved model for %s", h)
                except Exception as e:
                    logger.error("Error saving model for %s: %s", h, str(e))
    ...
    def prepare_features(self, data):
        ...
        features['rolling_mean'] = (
            features['count']
            .rolling(window=self.sequence_length, min_periods=1)
            .mean()
        )
